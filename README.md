...
│                                                                          │
│  ┌─────────────┐ ┌─────────────┐ ┌─────────────┐ ┌────────────────────┐  │
│  │  Firewall   │ │   Gateway   │ │   Bastion   │ │    Management      │  │
│  │ 10.0.1.0/24 │ │ 10.0.2.0/24 │ │ 10.0.3.0/26 │ │   10.0.4.0/24      │  │
│  │  (예약됨)   │ │  (예약됨)   │ │  (예약됨)   │ │ ┌────────────────┐ │  │
│  │             │ │             │ │             │ │ │  vm-jumpbox    │ │  │
│  │             │ │             │ │             │ │ │  10.0.4.4      │ │  │
│  │             │ │             │ │             │ │ │  [nsg-jumpbox] │ │  │
...
**Hub-Spoke 장점:**
- 중앙 집중식 보안 관리 (Firewall, Bastion, Jump Box)
- 네트워크 분리로 보안 강화
- 공유 서비스 효율적 관리
- 확장성 (새 Spoke 추가 용이)
...
│  │                                                                                       │  │
│  │   ┌─────────────────────────────────┐       ┌─────────────────────────────────────┐   │  │
│  │   │     vnet-hub (10.0.0.0/16)      │       │   vnet-spoke-aks (10.1.0.0/16)      │   │  │
│  │   │                                 │       │                                     │   │  │
│  │   │  ┌───────────────────────────┐  │       │  ┌─────────────────────────────┐    │   │  │
│  │   │  │ AzureFirewallSubnet       │  │       │  │ snet-aks-system (10.1.1.0/24)│    │   │  │
│  │   │  │ 10.0.1.0/24 (예약됨)      │  │       │  │  ┌─────────────────────┐    │    │   │  │
│  │   │  └───────────────────────────┘  │       │  │  │ aks-systempool-vmss │    │    │   │  │
│  │   │                                 │       │  │  │ Node: 10.1.1.4      │    │    │   │  │
│  │   │  ┌───────────────────────────┐  │       │  │  │ Pod1: 10.1.1.21     │    │    │   │  │
│  │   │  │ GatewaySubnet             │  │       │  │  │ Pod2: 10.1.1.32     │    │    │   │  │
│  │   │  │ 10.0.2.0/24 (예약됨)      │  │ VNet  │  │  └─────────────────────┘    │    │   │  │
│  │   │  └───────────────────────────┘  │Peering│  └─────────────────────────────┘    │   │  │
│  │   │                                 │◄─────►│                                     │   │  │
│  │   │  ┌───────────────────────────┐  │       │  ┌─────────────────────────────┐    │   │  │
│  │   │  │ AzureBastionSubnet        │  │       │  │ snet-aks-user (10.1.2.0/23) │    │   │  │
│  │   │  │ 10.0.3.0/26 (예약됨)      │  │       │  │ (워크로드 노드풀용)          │    │   │  │
│  │   │  └───────────────────────────┘  │       │  └─────────────────────────────┘    │   │  │
│  │   │                                 │       │                                     │   │  │
│  │   │  ┌───────────────────────────┐  │       │  ┌─────────────────────────────┐    │   │  │
│  │   │  │ snet-management           │  │       │  │ snet-appgw (10.1.5.0/24)    │    │   │  │
│  │   │  │ 10.0.4.0/24               │  │       │  │ [nsg-appgw]                 │    │   │  │
│  │   │  │  ┌─────────────────────┐  │  │       │  └─────────────────────────────┘    │   │  │
│  │   │  │  │ vm-jumpbox          │  │  │       │                                     │   │  │
│  │   │  │  │ Private: 10.0.4.4   │  │  │       │  ┌─────────────────────────────┐    │   │  │
│  │   │  │  │ Public: 4.217.191.222│  │  │       │  │ snet-aks-ilb (10.1.4.0/24)  │    │   │  │
│  │   │  │  │ [nsg-jumpbox]       │  │  │       │  │ (Internal LB용)             │    │   │  │
│  │   │  │  └─────────────────────┘  │  │       │  └─────────────────────────────┘    │   │  │
│  │   │  └───────────────────────────┘  │       │                                     │   │  │
│  │   │                                 │       │  ┌─────────────────────────────┐    │   │  │
...
│  │                                                                                       │  │
│  │   ┌─────────────────────────────────────────────────────────────────────────────┐    │  │
│  │   │                    acraksdemo66749 (Azure Container Registry)                │    │  │
│  │   │                    acraksdemo66749.azurecr.io                                │    │  │
│  │   └─────────────────────────────────────────────────────────────────────────────┘    │  │
│  └───────────────────────────────────────────────────────────────────────────────────────┘  │
│                                                                                             │
│  ┌───────────────────────────────────────────────────────────────────────────────────────┐  │
│  │                MC_rg-aks-network-demo_aks-demo-cluster_koreacentral                   │  │
│  │                              (AKS 관리형 리소스 그룹)                                   │  │
│  │                                                                                       │  │
│  │   ┌──────────────────┐  ┌──────────────────┐  ┌────────────────────────────────┐     │  │
│  │   │  kubernetes LB   │  │ Public IP        │  │ Public IP (Service)            │     │  │
│  │   │  (Standard)      │  │ (Outbound)       │  │ 4.230.74.213                   │     │  │
│  │   └────────┬─────────┘  │ 4.230.48.189     │  │ → aks-demo-service:80          │     │  │
│  │            │            └──────────────────┘  └────────────────────────────────┘     │  │
│  │            ▼                                                                         │  │
│  │   ┌──────────────────────────────────────────────────────────────────────────┐      │  │
│  │   │  aks-systempool-64021530-vmss (Virtual Machine Scale Set)                 │      │  │
│  │   │  [aks-agentpool-30771771-nsg]                                             │      │  │
│  │   └──────────────────────────────────────────────────────────────────────────┘      │  │
│  └───────────────────────────────────────────────────────────────────────────────────────┘  │
│                                                                                             │
└─────────────────────────────────────────────────────────────────────────────────────────────┘
...
AKS 클러스터에 대한 종합 모니터링 시스템이 구성되어 있습니다.
...
└──────────────────────────┼──────────────────────────────────────┘
           ...
           ▼               ▼               ▼
┌──────────────────┐ ┌──────────────────┐ ┌──────────────────┐
│  Log Analytics   │ │  Azure Monitor   │ │ Azure Managed    │
│  Workspace       │ │  Workspace       │ │ Grafana          │
│  (Logs/Metrics)  │ │  (Prometheus)    │ │ (Visualization)  │
└────────┬─────────┘ └────────┬─────────┘ └────────┬─────────┘
         │                    │                    │
         └────────────────────┼────────────────────┘
                              │
...
| 구성 요소 | 리소스명 | 용도 |
|----------|---------|------|
| Log Analytics 워크스페이스 | `law-aks-demo-monitoring` | 로그 및 메트릭 저장 |
| Container Insights | omsagent 애드온 | 컨테이너 로그/메트릭 수집 |
| Azure Monitor 워크스페이스 | `amw-aks-demo-prometheus` | Prometheus 메트릭 저장 |
| Azure Managed Grafana | `grafana-aks-demo` | 대시보드 시각화 |

### 9.3 알림 규칙

| 알림명 | 조건 | 심각도 | 설명 |
|--------|------|--------|------|
| `alert-aks-high-cpu` | 노드 CPU > 80% | Warning (2) | 15분간 평균 CPU 사용률 초과 시 |
| `alert-aks-high-memory` | 노드 메모리 > 80% | Warning (2) | 15분간 평균 메모리 사용률 초과 시 |
| `alert-aks-node-not-ready` | 노드 문제 감지 | Error (1) | 클러스터 노드 상태 이상 시 |

**알림 수신:** 액션 그룹 `ag-aks-demo-alerts`를 통해 이메일로 알림 전송
...
| **Prometheus Query** | https://amw-aks-demo-prometheus-ehbtbvdgf4dyb6gs.koreacentral.prometheus.monitor.azure.com |

### 9.5 Grafana 기본 대시보드

Azure Managed Grafana에는 다음 기본 대시보드가 포함되어 있습니다:

| 대시보드 | 용도 |
|---------|------|
| Kubernetes / Cluster | 전체 클러스터 상태 |
| Kubernetes / Nodes | 노드별 리소스 사용량 |
| Kubernetes / Pods | Pod 상태 및 메트릭 |
| Kubernetes / Workloads | Deployment, StatefulSet 등 |
...
# 알림 규칙 목록 확인
az monitor metrics alert list --resource-group rg-aks-network-demo -o table

# Log Analytics에서 컨테이너 로그 쿼리 (Azure Portal)
ContainerLog
